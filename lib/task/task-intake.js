...
function statusUpdateEventJson(taskId, contextId, state, messageText) {
    const a2aState = {
    ...
    return TaskStatusUpdateEvent.toJSON(TaskStatusUpdateEvent.fromJSON({ taskId, contextId, status }));
}
export class TaskIntake {
    ...
    contextByRuntimeAgentId = new Map();
    ...
    onPayloadDispatched(envelope) {
        ...
        let message;
        try {
            message = Message.fromJSON(envelope.payload.a2a);
        }
        catch (error) {
        ...
        void this.accept(request).catch(async (error) => {
            ...
            try {
                ...
                await this.reportWith(request.requestedTaskId ?? "(unresolved)", request.contextId, { kind: STATUS_REPORTS.FAILED, message: `payload accept failed: ${detail}` }, correlationId);
            }
            catch (reportError) {
    ...
    beginAndQueue(taskId, contextId, channel, prompt, correlationId) {
    ...
    async executeTurn(contextId, taskId, prompt) {
        ...
        try {
            ...
            if (outcome.finishReason === "blocked") {
                ...
                this.settle(taskId, "failed", undefined, "input-required-unsupported", "agent requested additional input, but ClusterLink payload messages do not support resuming a held task");
                return;
            }
    ...
    async emitStatus(taskId, state, messageText) {
        const record = this.registry.get(taskId);
        if (!record)
            return;
        ...
        if (isTerminalState(state))
            this.registry.completeDispatch(record.correlationId, envelope);
        await this.hub.reportPayload(envelope);
    }
    async acquireSessionHandle(contextId, cwd) {
    ...
    contextIdForRuntimeAgent(runtimeAgentId) {
    ...
    async reportWith(taskId, contextId, event, fallbackCorrelationId) {
        const record = this.registry.get(taskId);
        ...
        try {
            ...
            for (const context of this.contexts.list()) {
                if (context.activeTaskId || context.queuedTaskIds.length > 0)
                    busy.add(context.contextId);
            }
            const disposed = [];
            for (const [contextId, handle] of [...this.handles]) {
                if (busy.has(contextId))
                    continue;
            ...
            for (const contextId of disposed)
                this.log.info("intake", `disposed idle runtime handle for context ${contextId} (cap ${keep})`);
        }
        catch (error) {
