...
export class TaskIntake {
    ...
    async accept(taskId, prompt, metadata, source, contextId, workspaceHint) {
        ...
        try {
            ...
            if (existingHandle)
                this.log.info("intake", `task ${taskId} joins existing conversation ${resolvedContextId}`, taskId);
            ...
            if (!accepted)
                this.log.warn("intake", `started event not delivered (hub offline); session is running`, taskId);
    ...
    enqueueRun(sessionKey, taskId, handle, prompt) {
